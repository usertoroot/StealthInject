...
HMODULE GetRemoteModuleHandle(const char* module)
{
	...
	codecaveAddress = VirtualAllocEx(_hProcess, 0, 1024, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	...
	VirtualProtectEx(_hProcess, codecaveAddress, workspaceIndex, PAGE_EXECUTE_READWRITE, &oldProtect);

	// Write out the patch
	WriteProcessMemory(_hProcess, codecaveAddress, workspace, workspaceIndex, &bytesRet);

	// Restore page protection
	VirtualProtectEx(_hProcess, codecaveAddress, workspaceIndex, oldProtect, &oldProtect);

	// Make sure our changes are written right away
	FlushInstructionCache(_hProcess, codecaveAddress, workspaceIndex);

	// Free the workspace memory
	HeapFree(GetProcessHeap(), 0, workspace);

	hThread = CreateRemoteThread(_hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)((void*)codecaveExecAddr), 0, 0, NULL);
	...
	VirtualFreeEx(_hProcess, codecaveAddress, 0, MEM_RELEASE);
	return (HMODULE)address;
}

FARPROC RemoteGetProcAddress(HMODULE hModule, const char* func)
{
	// Main DLL we will need to load
	HMODULE kernel32	= NULL;
...
bool ReadMemory(LPVOID baseAddress, LPVOID buffer, int size)
{
	static DWORD bytesRead;
	return ReadProcessMemory(_hProcess, baseAddress, buffer, size, &bytesRead) == TRUE;
}

bool WriteMemory(LPVOID baseAddress, LPVOID buffer, int size)
{
	static DWORD bytesWritten;
	return WriteProcessMemory(_hProcess, baseAddress, buffer, size, &bytesWritten) == TRUE;
}
...
void LoadSections()
{
	...
	{
		ReadMemory(sectionPointer, &section, sizeof(IMAGE_SECTION_HEADER));
		...
		WriteMemory((DWORD)&section.Misc.PhysicalAddress - (DWORD)&section + (char*)sectionPointer, &sectionAddress, sizeof(DWORD));

		if (section.SizeOfRawData)
			WriteMemory(sectionAddress, _dllData + section.PointerToRawData, size);
		else
		{
			char* dummy = new char[size];
			memset(dummy, 0, size);
			WriteMemory(sectionAddress, dummy, size);
			delete[] dummy;
		}
	}
}
...
void Relocate(int delta)
{
	...
	ReadMemory(relocationPointer, &relocation, sizeof(_IMAGE_BASE_RELOCATION));
	...
	while (relocation.VirtualAddress)
	{
		...
		for (DWORD i = 0; i < (relocation.SizeOfBlock - sizeof(_IMAGE_BASE_RELOCATION)) / 2; i++, relocationInformationPointer++) 
		{
			...
			ReadMemory(relocationInformationPointer, &relocationInformation, sizeof(unsigned short));
			...
			case IMAGE_REL_BASED_HIGHLOW:
				{
					...
					ReadMemory(addrPointer, &addr, sizeof(DWORD));

					addr += delta;
					WriteMemory(addrPointer,&addr, sizeof(DWORD));
					break;
				}
			default:
				break;
...
void ResolveImports()
{
	...
	ReadMemory(importDescriptorPointer, &importDescriptor, sizeof(_IMAGE_IMPORT_DESCRIPTOR));

	while (importDescriptor.Name) 
	{
		ReadMemory(_dllStart + importDescriptor.Name, &libraryName, sizeof(libraryName));

		HMODULE handle = (HMODULE)Inject(_hProcess, libraryName);
		if (handle == NULL) 
			return;
		...
		ReadMemory(thunkRefPointer, &thunkRef, sizeof(DWORD));

		while (thunkRef) 
		{
			if (IMAGE_SNAP_BY_ORDINAL(thunkRef)) 
				ReadMemory((LPVOID)IMAGE_ORDINAL(thunkRef), libraryName, sizeof(libraryName));
			else 
			{
				PIMAGE_IMPORT_BY_NAME thunkDataPointer = (PIMAGE_IMPORT_BY_NAME)(_dllStart + thunkRef);
				IMAGE_IMPORT_BY_NAME thunkData;
				ReadMemory(thunkDataPointer, &thunkData, sizeof(IMAGE_IMPORT_BY_NAME));
				ReadMemory((DWORD)&thunkData.Name - (DWORD)&thunkData + (char*)thunkDataPointer, libraryName, sizeof(libraryName));
			}

			funcRef = (FARPROC)RemoteGetProcAddress(handle, libraryName);
			if (funcRef == 0)
				return;

			WriteMemory(funcRefPointer, &funcRef, sizeof(FARPROC));

			thunkRefPointer++;
			funcRefPointer++;

			ReadMemory(thunkRefPointer, &thunkRef, sizeof(DWORD));
			ReadMemory(funcRefPointer, &funcRef, sizeof(FARPROC));
		}
...
bool StealthLoadLibraryMemory(void* address)
{
	...
	//MainModule
	IMAGE_DOS_HEADER* mainDosHeaderPointer = (IMAGE_DOS_HEADER*)GetRemoteModuleHandle(NULL);
	ReadMemory(mainDosHeaderPointer, &_mainDosHeader, sizeof(IMAGE_DOS_HEADER));
	if (_mainDosHeader.e_magic != IMAGE_DOS_SIGNATURE)
		return false;

	ReadMemory((char*)mainDosHeaderPointer + _mainDosHeader.e_lfanew, &_mainNtHeaders, sizeof(IMAGE_NT_HEADERS));
	if (_mainNtHeaders.Signature != IMAGE_NT_SIGNATURE)
		return false;

	_dllStart = (char*)VirtualAllocEx(_hProcess, (LPVOID)(_injectingNtHeaders.OptionalHeader.ImageBase), _injectingNtHeaders.OptionalHeader.SizeOfImage, MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	if (!_dllStart)
	{
		_dllStart = (char*)VirtualAllocEx(_hProcess, NULL, _injectingNtHeaders.OptionalHeader.SizeOfImage, 	MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		if (!_dllStart)
			return false;
	}

	VirtualAllocEx(_hProcess, _dllStart, _injectingNtHeaders.OptionalHeader.SizeOfImage, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	...
	WriteMemory(headers, _dllData, _injectingDosHeader.e_lfanew + _injectingNtHeaders.OptionalHeader.SizeOfHeaders);
	...
	WriteMemory((DWORD)&_injectingNtHeaders.OptionalHeader.ImageBase - (DWORD)&_injectingNtHeaders + (char*)_injectedNtHeaders, &_dllStart, sizeof(DWORD));
	...
	if (_injectingNtHeaders.OptionalHeader.AddressOfEntryPoint)
	{
		...
		BYTE localPage[256];
		int codeIndex = 0;
		void* page = VirtualAllocEx(_hProcess, NULL, 256, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
		...
		WriteMemory(page, localPage, 256);

		FlushInstructionCache(_hProcess, page, codeIndex);

		HANDLE hThread = CreateRemoteThread(_hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)((void*)page), 0, 0, NULL);
		WaitForSingleObject(hThread, INFINITE); 

		DWORD result;
		GetExitCodeThread(hThread, &result);
		VirtualFreeEx(_hProcess, page, 0, MEM_RELEASE);
...
void StealthLoadLibrary(const char* path)
{
	...
	StealthLoadLibraryMemory(dllData);
	delete[] dllData;
}
